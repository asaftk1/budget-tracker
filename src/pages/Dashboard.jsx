...
export default function Dashboard() {
    ...
    const totalIncome = transactions.filter(tx => tx.type === 'הכנסה').reduce((acc, tx) => acc + tx.amount, 0);
    const totalExpense = transactions.filter(tx => tx.type === 'הוצאה').reduce((acc, tx) => acc + tx.amount, 0);
    ...
    const chartData = Array.from({ length: 6 }, (_, i) => {
        ...
        const income = transactions.filter(tx => tx.type === 'הכנסה' && new Date(tx.date).getMonth() === date.getMonth())
            .reduce((sum, tx) => sum + tx.amount, 0);

        const expense = transactions.filter(tx => tx.type === 'הוצאה' && new Date(tx.date).getMonth() === date.getMonth())
            .reduce((sum, tx) => sum + tx.amount, 0);
                ...
                <Card>
                    ...
                    <CardContent>
                        <ul className="space-y-3">
                            {[...transactions]
                                .sort((a, b) => new Date(b.date) - new Date(a.date)) // מיון לפי תאריך יורד
                                .slice(0, 5) // לקחת את 5 האחרונות (הכי חדשות)
                                .map((tx) => (
                                    <li key={tx.id} className="flex justify-between">
                                        ...
                                        <span className={`font-bold ${tx.type === 'הכנסה' ? 'text-green-600' : 'text-red-500'}`}>
                                            {tx.type === 'הכנסה' ? '+' : '-'}₪{tx.amount.toFixed(2)}
                                        </span>
                                    </li>
                                ))}
                        </ul>
                                ...
                                <ul className="text-sm text-right max-h-[300px] overflow-y-auto pr-4">
                                    {pieData.map((entry, index) => (
                                        <li key={index} className="mb-1">
                                            ...
                                            ></span>
                                            <strong>{entry.name}</strong>: {(entry.value / totalExpenses * 100).toFixed(1)}%
                                        </li>
                                    ))}
                                </ul>
                            </div>
