                        ...
                        <DialogContent className="sm:max-w-[400px]">
                            ...
                            {step === 0 ? (
                                <div className="flex flex-col gap-4 mt-4">
                                    <Button className="bg-green-100 hover:bg-green-200 text-lg" onClick={() => { setType('הכנסה'); setStep(1); }}>💰 הכנסה</Button>
                                    <Button className="bg-red-100 hover:bg-red-200 text-lg" onClick={() => { setType('הוצאה'); setStep(1); }}>💸 הוצאה</Button>
                                </div>
                            ) : (
                                <div className="space-y-3 mt-4">
                                    <div>
                        ...
                        <table className="w-full text-right border border-gray-200 text-sm border-collapse">
                            <thead>
                                <tr className="bg-gray-100 border-b text-right">
                                    <th className="p-2 w-1/4">קטגוריה</th>
                                    <th className="p-2 w-1/4">סכום</th>
                                    <th className="p-2 w-1/4">סוג</th>
                                    <th className="p-2 w-1/4 text-left">🗑️</th>
                                </tr>
                            </thead>
                            <tbody>
                                {groupedByDate[date].map((tx, i) => (
                                    <tr key={tx.id} className={i % 2 === 0 ? 'bg-gray-50' : ''}>
                                        <td className="p-2 w-1/4">{tx.category}</td>
                                        <td className="p-2 w-1/4">₪{tx.amount}</td>
                                        <td className={`p-2 w-1/4 font-bold ${tx.type === 'הכנסה' ? 'text-green-600' : 'text-red-500'}`}>
                                            {tx.type}
                                        </td>
